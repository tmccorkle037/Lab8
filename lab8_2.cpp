...
public:
    ...
    void scaleData(double scale) throw(int);
    void normalizeData();
    void centerData();
    void getStats();
    void menu();
...
Signal::Signal() {
    ...
    if(inFS.fail()){
        throw "ERROR: No such file, couldn't open\n-from default constructor";
    }
...
Signal::Signal(int fileNum) {
    ...
    if(length == 0){ //no data to process
        throw 0;
    }
...
void Signal::scaleData(double scale) throw(int){
...
void Signal::menu() {
    ...
    while (choice != 8) {
        ...
        try{
            ...
            cout << "Signal is empty, no data" << endl;
        }
        catch(...){
...
int main(int argc, char **argv) {
    ...
    while(opt != 0){
        ...
        cin >> num1 >> num2;
        cout << "num 1: " << num1 << " num2: " << num2 << endl; //testing to make sure the numbers are read in correctly
